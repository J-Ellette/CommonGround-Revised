...
| Layer | Choice | Notes |
|-------|--------|-------|
| Frontend | React + TypeScript | Same as before |
| Backend | Node.js (Fastify) | Same |
| Database | PostgreSQL (Supabase) | Hosted, no DevOps burden at launch |
| Auth | Supabase Auth (email + magic link) | Add passkeys/TOTP in a later milestone |
| File Storage | Supabase Storage | Upgrade to MinIO when self-hosting needed |
| Realtime | Supabase Realtime | Replaces Redis + WebSocket setup for now |
| Hosting | Vercel (frontend) + Railway or Render (API) | One-click deploys, no Kubernetes yet |
